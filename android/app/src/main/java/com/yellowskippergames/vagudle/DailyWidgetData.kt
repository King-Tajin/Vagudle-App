...
data class DailyRotationEntry(val wordLength: Int, val hardMode: Boolean)

private val DAILY_ROTATION = listOf(
...
fun rolledOverDailyWidgetData(previous: DailyWidgetData, newDate: String): DailyWidgetData {
...
fun parseDailyWidgetPayload(json: JSONObject): DailyWidgetData {
  ...
  val rankStatus = when (rankJson.getString("status")) {
...
fun saveDailyWidgetData(prefs: SharedPreferences, data: DailyWidgetData) {
  prefs.edit().apply {
